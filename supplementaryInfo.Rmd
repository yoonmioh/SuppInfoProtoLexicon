...
kable(xtable(summary(modelTableS1)$coef), digits=3, caption="Table S1: LME model for NMS confidence ratings across word frequency. All numeric variables in this model are centered.")
```
...
```{r Fig. 1, echo=FALSE, message=FALSE, warning=FALSE, evaluate=FALSE, fig.width=16, fig.height=6, fig.cap="Fig. 1: Mean rating for each stimulus per frequency bin with predicted rating for each density plot. Bin1 contains the most frequent words and Bin5 consists of the least frequent words."}
...
ggplot(allBins,aes(x = meanRating, y = as.factor(bin), fill = type)) +
    ...
    labs(linetype='Overall Mean',color='Overall Mean')
```

## Mean rating and phonotactic score for each stimulus per frequency bin
```{r Fig. S3, echo=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=10.5, fig.cap="Fig. S3: Mean rating and phonotactic score for each stimulus per frequency bin. The range of mean rating is represented on the y-axis and the x-axis represents the range of phonotactic score."}
...
```{r Fig. S4, echo=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=10.5, fig.cap="Fig. S4: Mean rating and phonotactic score for each stimulus per frequency bin (rematched pairs). The range of mean rating is represented on the y-axis and the x-axis represents the range of phonotactic score."}
...
The data is structured as follows:
  ...
  - *scoreDictNew* is the alternate phonotactice score based on the dictionary training set; statistics for trigrams, bigrams, and unigrams are jointly included.
  - *scoreRsNew* is the alternate phonotactice score based on the segmented running speech training set; statistics for trigrams, bigrams, and unigrams are jointly included.
  - *scoreUnsegNew* is the alternate phonotactice score based on the unsegmented running speech training set; statistics for trigrams, bigrams, and unigrams are jointly included.
